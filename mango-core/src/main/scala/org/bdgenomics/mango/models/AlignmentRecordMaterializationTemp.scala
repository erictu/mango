...
import org.bdgenomics.mango.tiling.AlignmentRecordTileTemp
 ...
 */
class AlignmentRecordMaterializationTemp(s: SparkContext,
                                     d: SequenceDictionary,
                                     parts: Int, chunkS: Int,
                                     refRDD: ReferenceRDD) extends LazyMaterialization[AlignmentRecord, AlignmentRecordTileTemp] with Serializable with Logging {
  ...
  override def get(region: ReferenceRegion, k: String): Option[IntervalRDD[ReferenceRegion, AlignmentRecordTileTemp]] = {
  ...
  /* If the RDD has not been initialized, initialize it to the first get request
	...
	*/
  override def multiget(region: ReferenceRegion, ks: List[String]): Option[IntervalRDD[ReferenceRegion, AlignmentRecordTileTemp]] = {
  ...
  override def put(region: ReferenceRegion, ks: List[String]) = {
    ...
    seqRecord match {
      case Some(_) => {
        ...
        val data = sc.parallelize(List((region, new AlignmentRecordTileTemp(sc, data, ref, reg, ks))))
